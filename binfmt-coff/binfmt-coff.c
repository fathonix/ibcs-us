...
#include <ibcs-us/linux26-compat/linux/coff.h>
...
static int	coff_parse_comments(struct file *, COFF_SCNHDR *, u_long *);
#ifndef	_KSL_IBCS_US
 ...
 */
static int
coff_parse_comments(struct file *fp, COFF_SCNHDR *sect, u_long *personality)
{
...
static inline unsigned long coff_mmap(
...
/*
 * ibcs-us just reuses the (argc, argv, envp) the kernel passed us when it
 * started us.
 */
#ifndef	_KSL_IBCS_US
static u32 *
...
found:
		if (lPers)
		  	personality = lPers;
		if ( (personality & 0xFF) == (current->personality & 0xFF) )
		  	set_personality(0);
	...
	if (bss.size != 0) {
		cap_mmap(1);
		down_write(&current->mm->mmap_sem);
		coff_mmap(NULL, PAGE_ALIGN(bss.vaddr),
			bss.size + bss.vaddr -
