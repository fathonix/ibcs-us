 ...
 *
 * This is a pruned version of the one in the linux-2.6.32 kernel.
 */
...
#include <ibcs-us/linux26-compat/linux/elf.h>
 ...
 */
static void
elf_set_personality(struct linux_binprm* binprm)
{
	long lPers, nPers;
	...
	if (nPers != 0 && nPers != current->personality) {
		lPers = nPers;
	}
...
static unsigned long elf_mmap(
    struct file* file, unsigned long addr,
    unsigned long len, unsigned long prot,
    unsigned long flag, unsigned long offset)
{
#ifdef	_KSL_IBCS_US
	return binfmt_mmap(file, addr, len, prot, flag, offset);
#else
	return do_mmap(file, addr, len, prot, flag, offset);
#endif
}
		...
		unsigned long total_size)
{
	...
	down_write(&current->mm->mmap_sem);
	map_addr = elf_mmap(filep, addr, size, prot, type, off);
...
static int load_elf_binary(struct linux_binprm *bprm, struct pt_regs *regs)
{
	...
	size_t type_pruned_pointer_no_more;
	struct {
	...
	/* Get the exec-header */
	type_pruned_pointer_no_more = (size_t)bprm->buf;
	loc->elf_ex = *(struct elfhdr *)type_pruned_pointer_no_more;
	...
	for(i = 0, elf_ppnt = elf_phdata;
		...
		if (elf_brk > elf_bss) {
			...
			if (nbyte) {
				...
				char* elf_bss_bias =
				    (char*)elf_bss + load_bias;
				if (clear_user((void __user *)elf_bss_bias,
							nbyte)) {
					/*
	...
	if (current->personality & MMAP_PAGE_ZERO) {
		...
		down_write(&current->mm->mmap_sem);
		error = elf_mmap(NULL, 0, PAGE_SIZE, PROT_READ | PROT_EXEC,
