...
#include <ibcs-us/per-svr4/types.h>
...
#include <ibcs-us/linux26-compat/linux/moduleparam.h>
...
abi_time(void)
{
	return svr4_y2k_send(SYS(time,0));
}
...
{
#if	_KSL_IBCS_US
  	_DX(regs) = (int)SYS(getppid);
	return (int)SYS(getpid);
#else
#if _KSL > 24
...
{
#if	_KSL_IBCS_US
  	_DX(regs) = (int)SYS(geteuid);
	return (int)SYS(getuid);
#else
#if _KSL > 28
...
{
#if	_KSL_IBCS_US
  	_DX(regs) = (int)SYS(getegid);
	return (int)SYS(getgid);
#else
#if _KSL > 28
...
#if _KSL_IBCS_US
  	/*
	 * The above comment about trailing /'s ain't true now: linux
	 * accepts them.
	 */
        return SYS(mkdir, fname, mode);
#else
	mm_segment_t		fs;
	char			*tmp, *p;
	int			error;
...
{
	int i;
	char buf[sizeof(*regs) * 3 + 5];
	char* p = buf;
	unsigned char* s = (unsigned char*)regs;
	for (i = sizeof(*regs); i > 0; i -= 1) {
	     	int d = *s >> 4;
		*p++ = d < 10 ? d + '0' : d + ('a' - 10);
	     	d = *s++ & 0xF;
		*p++ = d < 10 ? d + '0' : d + ('a' - 10);
		*p++ = (s - (unsigned char*)regs) % 4 == 0 ? '.' : ' ';
	}
	p[-1] = '\0';
	printk("%s\n", buf); 
	return (p - buf + 1);
