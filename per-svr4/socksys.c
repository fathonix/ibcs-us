...
#include <ibcs-us/per-svr4/socksys.h>
...
#include <ibcs-us/linux26-compat/linux/if_arp.h>
...
#include <ibcs-us/linux26-compat/linux/route.h>
...
#ifdef	_KSL_IBCS_US
static int	socksys_chrdev_open(const char* filename, int mode, int flags);
#endif
#ifdef CONFIG_65BIT
...
#endif
#if _KSL_IBCS_US
	linux26_chrdev_open: socksys_chrdev_open,
#endif
};

#ifdef	_KSL_IBCS_US
/*
 * In the user space implementation register_chrdev() can't create new
 * character special files under /dev, ibcs-us fakes it by calling
 * file_operations->linux26_chrdev_open() above to see if we are interested.
 * If we are, we must return the inode->i_rdev.
 */
static int socksys_chrdev_open(const char* filename, int mode, int flags)
{
    if (strcmp(filename, "socksys") == 0) {
	return MKDEV(SOCKSYS_MAJOR, 0);
    }
    if (strcmp(filename, "spx") == 0) {
	return MKDEV(SOCKSYS_MAJOR, 1);
    }
#if 0
    if (strcmp(filename, "xti") == 0) {
	/*
	* TODO: recognise "xti" device names.  Minors 2..255 are for the xti
	* device, as described below.  I don't know what their device names
	* under /dev/ were.
	*/
	return MKDEV(SOCKSYS_MAJOR, 2..255);
    }
#endif
    return -ENOENT;
}
...
inherit_socksys_funcs(u_int fd, int state)
{
	 ...
	 */
	{
	  	struct socket   *sp = SOCKET_I(ip); /* inode -> socket */
		if (sp->sk) 
...
spx_connect(u_int fd, int spxnum)
{
	...
	set_fs(get_ds());
#ifdef CONFIG_65BIT
	err = SYS_NATIVE(connect, newfd, &sun, sizeof(struct sockaddr_un));
#else
...
socksys_getpeername(int fd, caddr_t arg)
{
	...
	set_fs(get_ds());
#ifdef CONFIG_65BIT
	error = SYS_NATIVE(getpeername, fd, &uaddr, &addrlen);
#else
...
socksys_getsockname(int fd, caddr_t arg)
{
	...
	set_fs(get_ds());
#ifdef CONFIG_65BIT
	error = SYS_NATIVE(getsockname, fd, &uaddr, &addrlen);
#else
	a[0]=fd; a[1]= (int)((long)(&uaddr)); a[2]= (int)((long)(&addrlen));
