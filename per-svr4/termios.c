...
#include <ibcs-us/per-svr4/termios.h>
...
svr_to_linux_termio(int fd, int op, struct svr_termio *it)
{
	...
	if (!access_ok(VERIFY_READ, it, sizeof(struct svr_termio)))
		return -EFAULT;

	fs = get_fs();
	set_fs(get_ds());
	error = SYS_NATIVE(ioctl,fd, TCGETA, (long)&t);
	set_fs(fs);
	...
	set_fs(get_ds());
	error = SYS_NATIVE(ioctl,fd, op, (long)&t);
	set_fs(fs);
...
linux_to_svr_termio(int fd, struct svr_termio *it)
{
	...
	if (!access_ok(VERIFY_WRITE, it, sizeof(struct svr_termio)))
		return -EFAULT;
...
svr4_to_linux_termios(int fd, int op, struct svr4_termios *it)
{
	...
	if (!access_ok(VERIFY_READ, it, sizeof(struct svr4_termios)))
		return -EFAULT;

	fs = get_fs();
	set_fs(get_ds());
	error = SYS_NATIVE(ioctl,fd, TCGETS, (long)&t);
	set_fs(fs);
	...
	if ((t.c_lflag & 0100000))
		{SYS_NATIVE(ioctl,fd, TIOCEXCL, 0);}
	else
		{SYS_NATIVE(ioctl,fd, TIOCNXCL, 0);}
...
linux_to_svr4_termios(int fd, int op, struct svr4_termios *it)
{
	...
	if (!access_ok(VERIFY_WRITE, it, sizeof(struct svr4_termios)))
		return -EFAULT;
...
int err;
	...
	case 5: /* TCSBRK  (TIOC|5) */
		err = SYS(ioctl,fd, TCSBRK, (long)data); return err;
	case 6: /* TCXONC  (TIOC|6) */
		err = SYS(ioctl,fd, TCXONC, (long)data); return err;
	case 7: /* TCFLSH  (TIOC|7) */
		err = SYS(ioctl,fd, TCFLSH, (long)data); return err;
	...
	case 20: /* TCSETPGRP  (TIOC|20) set pgrp of tty */
		err = SYS(ioctl,fd, TIOCSPGRP, (long)data); return err;
	case 21: /* TCGETPGRP  (TIOC|21) get pgrp of tty */
		err = SYS(ioctl,fd, TIOCGPGRP, (long)data); return err;
	...
	case 103: /* TIOCSWINSZ (TIOC|103) */
		err = SYS(ioctl,fd, TIOCSWINSZ, (long)data); return err;
	case 104: /* TIOCGWINSZ (TIOC|104) */
		err = SYS(ioctl,fd, TIOCGWINSZ, (long)data); return err;
...
svr4_termiox_ioctl(int fd, u_int cmd, caddr_t data)
{
	...
	if (!access_ok(cmd == 1 ? VERIFY_WRITE : VERIFY_READ,
	...
	switch (cmd) {
		case 2: /* TCSETX */
			error = SYS_NATIVE(ioctl,fd, TCSETS, (long)&t);
			break;
		case 3: /* TCSETXW */
			error = SYS_NATIVE(ioctl,fd, TCSETSW, (long)&t);
			break;
		case 4: /* TCSETXF */
			error = SYS_NATIVE(ioctl,fd, TCSETSF, (long)&t);
			break;
	}
	set_fs(fs);
	return (error);
...
bsd_to_linux_termios(int fd, int op, struct bsd_termios *it)
{
	...
	if (!access_ok(VERIFY_READ, it, sizeof(struct bsd_termios)))
		return -EFAULT;
...
linux_to_bsd_termios(int fd, int op, struct bsd_termios *it)
{
	...
	if (!access_ok(VERIFY_WRITE, it, sizeof(struct bsd_termios)))
		return -EFAULT;
...
int
bsd_ioctl_termios(int fd, unsigned int func, void *data)
{
int error;
	switch (func & 0xff) {
		case 0:	 {				/* TIOCGETD */
			...
			old_fs = get_fs();
			set_fs(get_ds());
			error = SYS_NATIVE(ioctl,fd, TIOCGETD, (long)&ldisc);
			set_fs(old_fs);
			if (!error)
		...
		case 1: {				/* TIOCSETD */
			...
			old_fs = get_fs();
			set_fs(get_ds());
			error = SYS_NATIVE(ioctl,fd, TIOCSETD, (long)&ldisc);
			set_fs(old_fs);
			return error;
		}
		...
		case 8: {				/* TIOCGETP */
			...
			if (!access_ok(VERIFY_WRITE, data, sizeof(sg)))
				return -EFAULT;
...
/* O_CBREAK or O_RAW */		? 0 : ((t.c_lflag & ISIG) ? 0x02 : 0x20))
		...
		case 10: {				/* TIOCSETN */
			...
			if (!access_ok(VERIFY_READ, data, sizeof(sg)))
				return -EFAULT;
		...
		case 17: {				/* TIOCSETC */
			...
			if (!access_ok(VERIFY_READ, data, sizeof(tc)))
				return -EFAULT;
		...
		case 18: {				/* TIOCGETC */
			...
			if (!access_ok(VERIFY_WRITE, data, sizeof(tc)))
				return -EFAULT;
		...
		case 13:				/* TIOEXCL */
			error=SYS(ioctl,fd, TIOCEXCL, (long)data);return error;

		case 14:				/* TIOCNXCL */
			error=SYS(ioctl,fd, TIOCNXCL, (long)data);return error;

		case 16:				/* TIOCFLUSH */
			error=SYS(ioctl,fd, TCFLSH, (long)data);return error;
		...
		case 26:				/* TIOCGETD */
			error=SYS(ioctl,fd, TIOCGETD, (long)data);return error;

		case 27:				/* TIOCSETD */
			error=SYS(ioctl,fd, TIOCSETD, (long)data);return error;

		case 97:				/* TIOCSCTTY */
			error=SYS(ioctl,fd, TIOCSCTTY, (long)data);return error;

		case 103:				/* TIOCSWINSZ */
			error=SYS(ioctl,fd, TIOCSWINSZ, (long)data);return error;

		case 104:				/* TIOCGWINSZ */
			error=SYS(ioctl,fd, TIOCGWINSZ, (long)data);return error;

		case 113:				/* TIOCNOTTY */
			error=SYS(ioctl,fd, TIOCNOTTY, (long)data);return error;

		case 118:	 			/* TIOCSPGRP */
			error=SYS(ioctl,fd, TIOCSPGRP, (long)data);return error;

		case 119:				/* TIOCGPGRP */
			error=SYS(ioctl,fd, TIOCGPGRP, (long)data);return error;

		case 123:				/* TIOCSBRK */
			error=SYS(ioctl,fd, TCSBRK, (long)data); return error;
