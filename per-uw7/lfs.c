...
#include <ibcs-us/per-uw7/resource.h>
#include <ibcs-us/per-uw7/statvfs.h>
...
uw7_statvfs64(char *filename, struct uw7_statvfs64 *stvfsp)
{
	...
	if (!error) {
#ifdef _KSL_IBCS_US
		struct super_block	*sbp = inode_get_i_sb(path.dentry->d_inode, &path);
#else
#if _KSL > 24
#if _KSL > 26
...
uw7_fstatvfs64(int fd, struct uw7_statvfs64 *stvfsp)
{
	...
	struct kstatfs		tmp;
	struct super_block	*sbp = (struct super_block*)0;
#ifdef _KSL_IBCS_US
	struct path		path;
	error = linux26_user_fpath(fd, &path);
	if (!error) {
		sbp = inode_get_i_sb(fp->f_dentry->d_inode, &path);
		error = vfs_statfs(sbp->s_root, &tmp);
	}
#else
	sbp = fp->f_dentry->d_inode->i_sb;
	error = 0;
#endif
...
uw7_rlim64_to_user(int resource, struct uw7_rlim64 *rlimp)
{
	...
	int			error;
	error = SYS(getrlimit, resource, lxrlim);
	if (error) {
	    return error;
	}
#endif
...
uw7_setrlimit64(int resource, const struct uw7_rlim64 *rlimp)
{
	...
	return 0;
#else
	return SYS(setrlimit, resource, lxrlim);
#endif
