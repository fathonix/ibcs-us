...
#include <ibcs-us/per-uw7/types.h>
#include <ibcs-us/per-uw7/stat.h>
...
report_uw7_stat64(struct kstat *stp, struct uw7_stat64 *bufp)
{
	...
	buf.st_rdev	= linux_to_uw7_dev_t(abi_map(stp->rdev,2));
	buf.st_size	= stp->size;

	buf.st_atime.tv_sec = svr4_y2k_send(stp->atime.tv_sec);
	buf.st_mtime.tv_sec = svr4_y2k_send(stp->mtime.tv_sec);
	buf.st_ctime.tv_sec = svr4_y2k_send(stp->ctime.tv_sec);
